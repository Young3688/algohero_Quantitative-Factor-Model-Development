...
# (Ts_ArgMax(SignedPower(((returns < 0) ? stddev(returns, 4) : close), 2.), 4)) - 0.5

# Alpha#2:
# (-1 * correlation(rank(delta(log(volume), 2)), rank(((close - open) / open)), 6))
# (-1 * correlation(delta(log(volume), 2), ((close - open) / open), 4))

# Alpha#3:
# (-1 * correlation(rank(open), rank(volume), 10))
# (-1 * correlation(open, volume, 4))
# (-1 * correlation(sma(open), delta(volume, 1) / volume, 4))

# Alpha#4:
# (-1 * Ts_Rank(rank(low), 9))
# (-1 * Ts_Rank((low / close.shift(4) - 1), 4))

# Alpha#5:
# (rank((open - (sum(vwap, 10) / 10))) * (-1 * abs(rank((close - vwap)))))
# ((open - (sum(vwap, 4) / 4)) * (-1 * (close - vwap)))
...
# ((sma(close, 24) < volume) ? ((-1 * ts_rank(abs(delta(close, 4)), 4)) * sign(delta(close, 4))) : (-1* 1))

# Alpha#8:
# (-1 * rank(((sum(open, 5) * sum(returns, 5)) - delay((sum(open, 5) * sum(returns, 5)), 10))))
# (-1 * ((sum(open, 4) * sum(returns, 4)) - delay((sum(open, 4) * sum(returns, 4)), 4)))

# Alpha#9:
# ((0 < ts_min(delta(close, 1), 5)) ? delta(close, 1) : ((ts_max(delta(close, 1), 5) < 0) ? delta(close, 1) : (-1 * delta(close, 1))))
# ((0 < ts_min(delta(close, 1), 4)) ? delta(close, 1) : ((ts_max(delta(close, 1), 4) < 0) ? delta(close, 1) : (-1 * delta(close, 1))))


# Alpha#10:   
# ((0 < ts_min(delta(close, 1), 4)) ? delta(close, 1) : (-1 * delta(close, 1)))

# Alpha#11:
# ((rank(ts_max((vwap - close), 3)) + rank(ts_min((vwap - close), 3))) * rank(delta(volume, 3)))
# ((ts_max((vwap - close), 4) + ts_min((vwap - close), 4)) * delta(volume, 2))

# Alpha#12:
# (sign(delta(volume, 1)) * (-1 * delta(close, 1)))
# (sign(delta(sma(volume, 4), 4)) * (-1 * delta(sma(close, 4), 4)))

# Alpha#13:
# (-1 * rank(covariance(rank(close), rank(volume), 5)))
# (-1 * covariance(close, volume, 4))

# Alpha#14:
# ((-1 * rank(delta(returns, 3))) * correlation(open, volume, 10))
# ((-1 * delta(returns, 2)) * correlation(sma(open, 4), sma(volume, 4), 4))
