...
def compute_returns(df, index_column='date_time', value_column='close', period_labels=None, cost = None):
    df = df.set_index(index_column)
    periods = generate_periods(period_labels)
    for label, period in periods.items():
        df[label] = df[value_column].pct_change(periods=period)
        df[label] = df[label].apply(lambda x: 1 if x > cost else (-1 if x < -cost else 0))
    return df