...
def basic_strategy_backtesting(df, cost):
    ...
    for pre in df.columns[prediction_start_index:]:
        ...
        median = df[pre].median()
        std_dev = df[pre].std()
        threshold_1 = median + std_dev
        threshold_2 = median - std_dev 

        df[f'{pre}_sta_rtn'] = 0

        position = df[pre].apply(lambda x: 1 if x > threshold_1 else (-1 if x < threshold_2 else 0)).shift(1)
